...
const JobDetails = () => {
  ...
  const { data } = useGetSingleJobQuery(id, {pollingInterval: 1000});
  const job = data?.data || [];
  const { handleSubmit, register, reset } = useForm();
  const [applyJob] = useApplyJobMutation();
  const [postQuestion] = useQuestionMutation();
  ...
  const handleApply = () => {
    ...
    if(job.applicants.find(i=>i.id===user._id)){
    ...
    applyJob(data);
    toast.success("Congress!! You Applied Successfully")
  };
    ...
    <div className="pt-14 grid grid-cols-12 gap-5">
      ...
      <div className="col-span-3">
        ...
        <div className="mt-5 rounded-xl bg-primary/10 p-5 text-primary space-y-5">
          ...
          <div>
          { user.role==="employer" && (<h1>Candidates Email</h1>)}
          <ul>
          { user.role==="employer" && (job?.applicants.map((i,index)=> <li key={i.email}>{index+1}. {i.email} </li>))}
          </ul>
          </div>
        </div>
      </div>
